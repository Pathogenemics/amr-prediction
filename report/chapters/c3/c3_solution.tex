...
The methodology of this study is organized as a staged determinant-based pipeline that transforms curated phenotype and genotype resources from NCBI Pathogen Detection\footnote{\url{https://www.ncbi.nlm.nih.gov/pathogens/}} into antibiotic-specific prediction tables. The data stage has three main goals. First, it defines a shared genotype-phenotype cohort for valid antibiotic-specific analysis. Second, it normalizes determinant-level genotype evidence into a biologically consistent representation. Third, it connects normalized genotype evidence back to phenotype under alternative biological scopes so that later comparisons change the scope of evidence rather than the underlying cohort or model architecture.
...
Phenotype processing starts from curated AST records available through the NCBI Pathogen Detection AST Browser\footnote{\url{https://www.ncbi.nlm.nih.gov/pathogens/ast}} and is restricted immediately to isolates that also appear in the genotype resource through a shared \texttt{BioSample} identifier. After cohort alignment, only rows with the required identifying and phenotype fields are retained, and only the labels \texttt{susceptible} and \texttt{resistant} are kept for downstream binary prediction. Any \texttt{(BioSample, Antibiotic)} pair that appears with both labels is removed as contradictory evidence.

If multiple phenotype rows remain for the same \texttt{(BioSample, Antibiotic)} pair after contradiction removal, the retained row is chosen by keeping the lowest MIC observation after explicit ordering of measurement signs. This rule makes duplicate resolution deterministic and keeps the final phenotype table aligned with the more conservative interpretation of resistance evidence. The cleaned phenotype data are then split into antibiotic-specific tables because every downstream prediction task is defined at the antibiotic level.
...
\end{tikzpicture}
\caption[Phenotype-processing flowchart.]{Flowchart summarizing the phenotype-cleaning procedure used to construct antibiotic-specific AST tables.}
...
Genotype processing starts from curated AMR determinant records available through NCBI MicroBIGG-E\footnote{\url{https://www.ncbi.nlm.nih.gov/pathogens/pathogens_help/}}. Exact duplicate rows are removed first. The remaining table is then collapsed to one row per \texttt{(BioSample, Element symbol)} so that the same determinant is not counted multiple times within one isolate merely because it was detected under multiple reporting methods.
...
\end{tikzpicture}
\caption[Genotype-processing flowchart.]{Flowchart summarizing the genotype-cleaning and determinant-normalization procedure.}
...
After phenotype cleaning and genotype normalization, connection is performed separately for each antibiotic-specific phenotype table. The normalized genotype rows belonging to isolates in that phenotype table are retained under one of three biological scopes: \textit{strict}, \textit{broad}, or \textit{all}. The scientific variable being changed at this stage is therefore the biological scope of determinant connection, not the classifier family used later.
...
One important special case is \texttt{trimethoprim-sulfamethoxazole}. For this combination drug, the strict scope is manually expanded so that matching can use biologically meaningful component subclasses rather than a literal compound token only. This makes the strict setting component-aware for that antibiotic and prevents an artificial loss of retained genotype evidence.

The motivation for comparing \textit{strict}, \textit{broad}, and \textit{all} is methodological rather than historical. A narrow subclass match is biologically conservative but can leave many isolates with little or no retained evidence. A broader family-level match can recover relevant determinant context within drug families. The all-scope representation then serves as the maximum-coverage reference for measuring how much predictive value lies outside the constrained biological matching rules.
